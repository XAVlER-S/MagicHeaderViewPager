...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch(action) {
            ...
            case MotionEvent.ACTION_DOWN: {
                // 丢弃掉非主要手指的down事件（Action_Pointer_Down），避免错误处理
                if(mIsBeingDragged) {
                    break;
                }
                if(isReadyForPull()) {
    ...
    protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs, int layoutId) {
        LoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode, getPullToRefreshScrollDirection(), attrs, layoutId);
        setHeaderTipsText(layout);
        layout.setVisibility(View.INVISIBLE);
        return layout;
    }

    /**
     * Used internally for {@link #getLoadingLayoutProxy(boolean, boolean)}. Allows derivative classes to include any extra
     * LoadingLayouts.
     */
    ...
    /**
     * This is implemented by derived classes to return the created View. If you need to use a custom View (such as a custom
     * ListView), override this method and return an instance of your custom class.
     * <p/>
     * Be sure to set the ID of the view in this method, especially if you're using a ListActivity or ListFragment.
     * @param context Context to create view with
     * @param attrs AttributeSet from wrapped class. Means that anything you include in the XML layout declaration will be routed to
     *        the created View
    ...
    /**
     * Implemented by derived class to return whether the View is in a state where the user can Pull to Refresh by scrolling from
     * the end.
        ...
        if(doScroll) {
            if(mShowViewWhileRefreshing) {
                ...
                switch(mCurrentMode) {
                    ...
                    default:
                    case PULL_FROM_START:
                        smoothScrollTo( - (int)(getHeaderSize() * LoadingLayout.SCALE_REFRESHING), listener);
                        break;
                }
            } else {
                smoothScrollTo(0);
    ...
    protected final void setHeaderScroll(int value) {
        ...
        if(USE_HW_LAYERS) {
            /**
             * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer Views
             * as they change often, which would negate any HW layer performance boost.
             */
    ...
    /**
     * Updates the View State when the mode has been set. This does not do any checking that the mode is different to current state
     * so always updates.
     */
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if( ! isInEditMode()) {
            mTipsArray = context.getResources().getStringArray(R.array.loading_progress_tips_array);
        }

        mHeaderLayout = createLoadingLayout(context, Mode.PULL_FROM_START, a, layoutId);

        mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a, layoutId);
    ...
    private void setHeaderTipsText(LoadingLayout layout) {
        if(mTipsArray != null && mRandom != null) {
            try {
                if(TextUtils.isEmpty(mTipsString)) {
                    mTipsString = mTipsArray[mRandom.nextInt(mTipsArray.length)];
                }
                layout.setTipsText(mTipsString);
            } catch(Exception e) {
//                Log.e(String.valueOf(e.getMessage()));
    ...
    private void pullEvent() {
        ...
        switch(mCurrentMode) {
            ...
            default:
                ...
                itemDimension = (int) (getHeaderSize() * LoadingLayout.SCALE_REFRESHING);
                break;
        }
    ...
    public static enum AnimationStyle {
        /**
         * This is the default for Android-PullToRefresh. Allows you to use any drawable, which is automatically rotated and used as
         * a Progress Bar.
         */
        ROTATE,

        /**
         * This is the old default, and what is commonly used on iOS. Uses an arrow image which flips depending on where the user
         * has scrolled.
         */
        FLIP;
        ...
        /**
         * Maps an int to a specific mode. This is needed when saving state, or inflating the view from XML where the mode is given
         * through a attr int.
    ...
    public static enum Mode {
        ...
        /**
         * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is either the Top or Left,
         * depending on the scrolling direction.
         */
        PULL_FROM_START(0x1),

        /**
         * Only allow the user to Pull from the end of the Refreshable View to refresh. The start is either the Bottom or Right,
         * depending on the scrolling direction.
         */
        PULL_FROM_END(0x2),
    ...
    /**
     * Listener that allows you to be notified when the user has started or finished a touch event. Useful when you want to append
     * extra UI events (such as sounds). See ( {@link PullToRefreshAdapterViewBase#setOnPullEventListener}.
     * @author Chris Banes
     */
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the
         *        user is pulling. Only useful when <var>state</var> is {@link State#PULL_TO_REFRESH} or
    ...
    /**
     * An advanced version of the Listener to listen for callbacks to Refresh. This listener is different as it allows you to
    ...
    public static enum State {
        ...
        /**
         * When the UI is being pulled by the user, and <strong>has</strong> been pulled far enough so that it will refresh when
         * released.
         */
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if(mStartTime == - 1) {
                ...
                /**
                 * We do do all calculations in long to reduce software float calculations. We use 1000 as it gives us good accuracy
    ...
    public int getBackWardPosition() {
        return mBackWardPosition;
    }

    public void setBackWardPosition(int backWardPosition) {
        mBackWardPosition = backWardPosition;
    }
    ...
    public void setScaleRefreshing(float scaleRefreshing) {
        if(mHeaderLayout != null) {
            mHeaderLayout.setScaleRefreshing(scaleRefreshing);
        }
    }
}
