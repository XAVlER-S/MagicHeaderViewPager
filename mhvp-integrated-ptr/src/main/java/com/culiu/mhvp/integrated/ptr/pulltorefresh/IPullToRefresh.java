...
public interface IPullToRefresh<T extends View> {
    ...
    /**
     * Demos the Pull-to-Refresh functionality to the user so that they are aware it is there. This could be useful when the user
     * first opens your app, etc. The animation will only happen if the Refresh View (ListView, ScrollView, etc) is in a state where
     * a Pull-to-Refresh could occur by a user's touch gesture (i.e. scrolled to the top/bottom).
    ...
    /**
     * Returns whether the Touch Events are filtered or not. If true is returned, then the View will only use touch events where the
     * difference in the Y-axis is greater than the difference in the X-axis. This means that the View will not interfere when it is
     * used in a horizontal scrolling View (such as a ViewPager).
     ...
     * The actual LoadingLayout(s) which will be affected, are chosen by the parameters you give.
     * <p />
     * You should not keep the result of this method any longer than you need it.
    ...
    /**
     * Gets whether Overscroll support is enabled. This is different to Android's standard Overscroll support (the edge-glow) which
    ...
    /**
     * Set the Touch Events to be filtered or not. If set to true, then the View will only use touch events where the difference in
     * the Y-axis is greater than the difference in the X-axis. This means that the View will not interfere when it is used in a
     * horizontal scrolling View (such as a ViewPager), but will restrict which types of finger scrolls will trigger the View.
    ...
    /**
     * Sets whether Overscroll support is enabled. This is different to Android's standard Overscroll support (the edge-glow). This
     * setting only takes effect when running on device with Android v2.3 or greater.
     * @param enabled - true if you want Overscroll enabled
     */
    public void setPullToRefreshOverScrollEnabled(boolean enabled);

    /**
     * Sets the Widget to be in the refresh state. The UI will be updated to show the 'Refreshing' view, and be scrolled to show
     * such.
     */
